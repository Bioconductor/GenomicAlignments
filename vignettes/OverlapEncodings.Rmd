...
Various tools are provided in the `{r Rpackage ("GenomicAlignments")}` package
...
BAM file {\tt untreated1\_chr4.bam} (located in the `{r
Rpackage("pasillaBamSubset"}` data package) contains single-end reads from the
``Pasilla'' experiment and aligned against the dm3 genome (see
`?untreated1_chr4` in the `{r Rpackage("pasillaBamSubset")}` package for more
...
Rpackage("GenomicAlignments")}` package to load the reads into a *GAlignments*
...
creating a \Rclass{ScanBamParam} object that we pass to `readGAlignments` (see
`?ScanBamParam` in the `{r Rpackage("Rsamtools")}` package for the details).
...
BAM file {\tt untreated3\_chr4.bam} (located in the `{r
Rpackage("pasillaBamSubset")}` data package) contains paired-end reads from the
``Pasilla'' experiment and aligned against the dm3 genome (see
`?untreated3_chr4` in the `{r Rpackage("pasillaBamSubset")} package for more
...
The `show` method for *GAlignmentPairs* objects displays two {\tt ranges}
columns, one for the *first* alignment in the pair (the left column), and one
for the *last* alignment in the pair (the right column). The {\tt strand} column
...
Rpackage("GenomicFeatures")}` package to make a *TxDb* object containing the dm3
...
``flipped'' alignments in 2 *GRangesList8 objects with:
...
* `startInFirstSpannedExon`: the 1-based start position of the
...
Only 0.2% of the alignment pairs in `U3.GALP` are ``almost splice
compatible'' with at least 1 transcript in `exbytx`.
...
Finally note that the ``query start in transcript'' values returned by
`extractQueryStartInTranscript` are also defined for ``almost splice
